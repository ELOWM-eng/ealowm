...
export default function ContentClient({ sessionId, version, config }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<'care' | 'card'>('care')
  const [selectedCard, setSelectedCard] = useState(0) // 카드뉴스 번호 (0-based)
  const [currentPage, setCurrentPage] = useState(0) // 현재 장
          ...
          <div className="animate-fade-up space-y-4">
            ...
            <div
              ...
              {currentPage > 0 && (
                <button
                  onClick={prevPage}
                  className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/30 text-white flex items-center justify-center hover:bg-black/50 transition-all">
                  ‹
                </button>
              )}
              {currentPage < totalPages - 1 && (
                <button
                  onClick={nextPage}
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/30 text-white flex items-center justify-center hover:bg-black/50 transition-all">
                  ›
                </button>
              )}
            </div>
